...
@python_task
def _plot_mpas_mesh_png(file_path: str, resolution: Optional[str] = None) -> str:
...
class MeshAgent:
    ...
    def __init__(
        ...
        mpas_tools_version : str, optional
            MPAS-Tools versionto clone, by default "2.0.0"
    ...
    def _write_hex_namelist(
        self, work_dir: Path, cell_spacing_km: float,
    ...
    def _write_project_hexes_namelist(
        ...
        """Write namelist.projections for project_hexes from config."""
        cell_spacing_km = self._parse_resolution_km(resolution)
        self._write_hex_namelist(
            work_dir,
            cell_spacing_km,
    ...
    def _write_create_region_spec(
        self, work_dir: Path, create_region_config: Dict[str, Any], name: str,
    ) -> Path:
    ...
    def _partition_mesh(self, mesh_path: str, num_ranks: int) -> str:
        ...
        return textwrap.dedent(
            ...
            {self.gpmetis_path} {mesh_path} {num_ranks}
    ...
    def _clone_mpas_tools(self) -> str:
        ...
        return textwrap.dedent(
            ...
            cd {self.work_dir}
            git clone --depth 1 --branch {self.mpas_tools_version} \
    ...
    def _create_region(
        ...
        return textwrap.dedent(
            ...
            # mv x1.{self.resolution_cells[resolution]}.regional* {output_dir}/
            echo "Completed create_region at $(date)"
            """
        )

    # -------------------------------------------------------------------------
    # Private bash tasks — Mesh operations
    # -------------------------------------------------------------------------

    @bash_task
    def _download_global_mesh(self, resolution: str) -> str:
        ...
        return textwrap.dedent(
            f"""\
            set -eu -o pipefail
            echo "Started global mesh download at $(date)"
            echo "Executing on $(hostname)"
            mkdir -p {self.mesh_data_dir}
            cd {self.mesh_data_dir}
            wget -T 60 -t 3 {self.mesh_data_url}/x1.{self.resolution_cells[resolution]}_static.tar.gz
            tar -xzf x1.{self.resolution_cells[resolution]}_static.tar.gz
            rm -f x1.{self.resolution_cells[resolution]}_static.tar.gz
            echo "Completed global mesh download at $(date)"
            """
        )
    ...
    @agent_action
    async def download_metis(self) -> None:
        """Download Metis source code from GitHub."""
        self.log_dir.mkdir(parents=True, exist_ok=True)

        future = self._download_metis(
            executor=["service"],
    ...
    @agent_action
    async def build_metis(self) -> None:
        """Build gpmetis utility. Requires download_metis() first."""
        if not self.metis_downloaded:
            raise RuntimeError("Must call download_metis() before build_metis()")

        self.log_dir.mkdir(parents=True, exist_ok=True)

        future = self._build_metis(
            executor=["compute"],
    ...
    @agent_action
    async def install_mesh_tools(self) -> None:
    ...
    async def install(self) -> None:
        ...
        la_future = self._install_limited_area(
            executor=["service"],
            stdout=str(self.log_dir / "limited_area_install.stdout"),
            stderr=str(self.log_dir / "limited_area_install.stderr"),
        )
        ...
        try:
            await asyncio.wrap_future(la_future)
        except BashExitFailure as e:
            raise RuntimeError(
                f"MPAS-Limited-Area install failed (exit {e.exitcode}), "
                f"see {self.log_dir}/limited_area_install.stderr"
    ...
    async def download_global_mesh(self, resolution: str) -> Dict[str, str]:
        ...
        future = self._download_global_mesh(
            resolution,
            executor=["service"],
            stdout=str(self.log_dir / "download_global_mesh.stdout"),
            stderr=str(self.log_dir / "download_global_mesh.stderr"),
        )
        try:
            await asyncio.wrap_future(future)
        except BashExitFailure as e:
            raise RuntimeError(
                f"Global mesh download failed (exit {e.exitcode}), "
                f"see {self.log_dir}/download_global_mesh.stderr"
            )
    ...
    async def project_hexes(
        ...
        if not self.mesh_tools_installed:
            raise RuntimeError(
                "Must call install_mesh_tools() or install() before "
                "project_hexes()"
            )
    ...
    async def partition_mesh(
        ...
        if not self.metis_built:
            raise RuntimeError(
                "Must call build_metis() or install() before partition_mesh()"
            )

        self.log_dir.mkdir(parents=True, exist_ok=True)

        is_single = isinstance(num_ranks, int)
        ranks = [num_ranks] if is_single else list(dict.fromkeys(num_ranks))
    ...
    async def _ensure_tools_installed(
        self, mesh_config: Dict[str, Any], needs_partition: bool = True,
    ) -> None:
        """Install only the tools required for the given mesh configuration."""
        ...
        if regional is not None:
            if "project_hexes" in regional:
                if not self.mesh_tools_installed:
                    install_tasks.append(self.install_mesh_tools())
    ...
    async def generate_mesh(self) -> Dict[str, Any]:
        ...
        -------
        dict
            - mesh: path to final mesh file
            - graph: path to graph.info file (if available)
        ...
        needs_partition = bool(init_ranks or forecast_ranks)
        await self._ensure_tools_installed(mesh_config, needs_partition)
        ...
        if regional is None:
            ...
            result["mesh"] = paths["static"]
            result["graph"] = paths["graph"]
