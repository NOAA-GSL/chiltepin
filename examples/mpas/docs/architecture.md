...
**Example**: MetisAgent doesn't know or care how WPSAgent works. It only knows it needs to partition a mesh file that was generated by MPASLimitedAreaAgent.

### 2. Independent Testing

Each agent can be tested in isolation without requiring other components to be available.

**Example**: You can test MetisAgent's partition functionality with sample mesh files, without needing to build MPAS or WPS.

### 3. Component Reusability

Agents can be reused in other workflows that need the same functionality.

**Example**: WPSAgent could be used in a WRF forecast workflow with minimal changes. MetisAgent could partition meshes for any model that uses Metis.
...
**Example**: 
- MetisAgent and WPSAgent run on `build-executor` (service nodes for compilation)
...
**Purpose**: Manage Metis graph partitioning library

**Lifecycle**:
1. Download Metis source from GitHub
2. Build `gpmetis` utility
3. Partition MPAS mesh files for specified MPI rank counts

**Why separate?**: Metis is an independent library that could be used with any mesh-based model, not just MPAS.
...
**Purpose**: Manage MPAS regional mesh generation tools

**Lifecycle**:
1. Download MPAS-Limited-Area source
2. Build `create_region` utility
3. Download global mesh files
4. Create regional CONUS mesh from global mesh

**Why separate?**: Mesh generation is a preprocessing step independent of the model itself. Could be used to generate meshes for various MPAS applications.
...
├─ metis.download_metis() + metis.build()
├─ wps.download_wps() + wps.build()
├─ mpas_la.download() + mpas_la.build()
└─ mpas.download_mpas() + mpas.build()

Phase 3: Mesh (SEQUENTIAL)
├─ mpas_la.download_global_mesh()
├─ mpas_la.create_conus_mesh()
├─ metis.partition_mesh(init_ranks)
└─ metis.partition_mesh(forecast_ranks)
...
mesh = await mpas_la_agent.create_conus_mesh(...)
partitioned = await metis_agent.partition_mesh(mesh["graph"], ...)
...
@chiltepin_agent()
class MetisAgent:
    """Focused on Metis only"""
    def download_metis(self): pass
    def build(self): pass
...
```python
# WRF workflow reuses WPSAgent and MetisAgent
wrf_workflow = WRFWorkflow(
    agents=[wps_agent, metis_agent, wrf_agent, upp_agent]
)
```
